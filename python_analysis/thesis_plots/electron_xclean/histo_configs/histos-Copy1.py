...
def fillHistos(events,h,samp,cut,info,sum_wgt=1):
    ...
    if info["type"] == "signal":
        ...
        # print ("genObj_pf:", (ak.to_list(genObj_pf)[:5]))
        # match_pf = ak.flatten(match_pf)
        # genObj_pf = ak.flatten(genObj_pf)
        # match_pf_passID = ak.values_astype(match_pf.passID,int)
        # print ("genObj_pf:", genObj_pf[:5])

       
        #Good example to understand list comprehension:
        # arr = ak.Array([[1, 2, 3], [4, 5], [6]])
        # print ("arr:", arr)

        # # Remove sublists containing '5'
        # new_data = [sublist for sublist in arr if 5 not in sublist]
        # new_arr = ak.Array(new_data)
        # print("new arr", new_arr)
       

        # match_pf = events[hasMatch_pf].Electron
        # print ("stage1:", len(match_pf))
        # # match_pf = match_pf[match_pf.genMatched & ~match_pf.hasLptMatch]   
        # # print ("stage2:", len(match_pf))
        # match_pf_passID = ak.values_astype(match_pf.passID,int)
        
        # genObj_pf_minus = ak.where(match_pf.matchType==-1,events[hasMatch_pf].GenEle,None)
        # print("genObj_pf_minus: ",(ak.to_list(genObj_pf_minus)[:10]))
        
        
        # #rebuilt_list = [[item for item in sublist if item is not None] for sublist in genObj_pf_minus]
        # rebuilt_list = ak.Array([sublist for sublist in genObj_pf_minus if None not in sublist])
        # print("rebuilt_list[:10]:",rebuilt_list[:10]) 
        # genObj_pf_minus_new = ak.flatten(rebuilt_list)
        # print ("Indices of None:", ak.local_index(
        # print (len(genObj_pf_minus_new))
        # print ("genObj_pf_minus_new:",ak.to_list(genObj_pf_minus_new[:10]))
        # match_pf_passID = ak.values_astype(match_pf.passID,int)
        # print ("match_pf_passID:", match_pf_passID[:10])
        # print ("len(match_pf_passID):",len(match_pf_passID))
        # print ("events.GenEle.pt:",len(events[hasMatch_pf].GenEle.pt))

        # genObj_pf_minus_new = [[x for x in sublist if x is not []]for sublist in rebuilt_list]
        # print ("new:",genObj_pf_minus_new[:5])
        
        
        # genObj_pf_plus = ak.where(match_pf.matchType==1,events[hasMatch_pf].GenPos,None)
        # # genObj_pf_plus = genObj_pf_plus[~(ak.is_none(genObj_pf_plus))]
        # # genObj_pf_plus = ak.flatten(genObj_pf_plus)
        # #print ("genObj_pf_plus:",ak.to_list(genObj_pf_plus))
        ...
        h.fill("match_ele_gen_pt",match_type='R',passID=match_pf_passID,pt=events.GenEle.pt,weight=1)   #IMP
        #h.fill("match_ele_gen_pt",match_type='R',passID=match_pf_passID,pt=genObj_pf_plus.pt,weight=1)
